...
void match (token expected) {
    if (input_token == expected) {
		...
		if (input_token == t_add ||input_token == t_sub || input_token == t_mul ||input_token == t_div){
			myString.append("(");
			relation = true;
			myString.append(names[expected] + string(" "));
		}
		...
		else if (input_token == t_fi || input_token == t_od || input_token == t_eof || input_token == t_gets) {}
		else{
			myString.append(names[expected] + string(" "));
		}
		cout << endl;
...
class Syntax_Error : public SyntaxErrorException{
    ...
    Syntax_Error(const token invalid_token)  {
        message = "Invalid " + string(names[input_token]) + "\n";
    }
    virtual const char* what() const throw() {
