/*******************************************************                        
  ...
  -- Then added my functions to make a producer                                          
*******************************************************/
...
************* end syscall shmget comment ******/
  ...
  ********* end syscall gettimeofday comment ******/
  ...
  while (1) {
    ...
    j = nrand48(xsub1) & 3;
    // locking the semaphores
    p(semid[PROD], j);

    // product value of ID and shared ring flavor
    serial[j]++;
    shared_ring -> flavor[j][in_ptr[j]] = serial[j];
    in_ptr[j] = (in_ptr[j] + 1) % NUMSLOTS;

    //print out the value generated and then finish up one loop
    v(semid[CONSUMER], j);
  }
}
