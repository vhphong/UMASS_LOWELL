...
void coalesce( void ) {
	...
	while( p_free != NULL ) {
		...
		if( (p_free -> free_space == YES) && (p_free -> next != NULL) ) {


			if( (p_free -> next -> free_space == YES) && (p_free -> next -> start_addr == 
...
int allocate_memory( int seq_num ) {
	...
	if( req_array[seq_num].sizeof_request <= total_free_space ) {
		...
		while( p_free != NULL ) {
			...
			else if( ( p_free->size > req_array[seq_num].sizeof_request ) && ( p_free->free_space == YES ) ){
				if( policy == 4 ){
...
int buddy_system( int seq_num ) {
	...
	if( req_array[seq_num].true_size <= total_free_space ) {
		...
		while( p_free != NULL ) {
			...
			if( ( p_free -> size > req_array[seq_num].true_size ) && ( p_free -> free_space == YES ) ){
