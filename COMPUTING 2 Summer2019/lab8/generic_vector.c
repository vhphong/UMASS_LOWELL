...
typedef struct generic_vector {
	int size;
	int capacity;
	Item* data;  //array of void pointers was void** data;(I
	Status(*item_assign)(Item*, Item);// (item on left, item right) assignment can fail
	void(*item_destroy)(Item*);//should know how to destroy items
}G_Vector;
...
Status generic_vector_push_back(G_VECTOR hVector, Item item) {
	...
	Item* temp;  //array of void pointers
	int i;
	...
	if (pVector->size >= pVector->capacity){
		...
		//from that point on everything is NULL
