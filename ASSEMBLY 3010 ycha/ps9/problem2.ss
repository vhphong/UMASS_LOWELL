...
;;; Procedures to implement uml:while as a special form in our new language

(define (eval-while exp env)
  (define (iter cond body)
    (if (null? cond)
	#f
	(if (true? (mc-eval (car cond) env))
	    (begin (mc-eval (car body) env)
             (iter cond body))
	    (iter '() body))))
  (iter (while-cond exp) (while-body exp)))

(define (while? exp) (tagged-list? exp 'uml:while))

(define (while-cond exp) (cdr exp))
