...
// it allocates an array of MY_STRING of capacity 1
Vector_String_Ptr vector_string_init_default(void){
    ...
    if (vector_struct) {
        ...
        vector_struct->data = (MY_STRING*)malloc(sizeof(MY_STRING)*1);
...
// 1- you need to loop through the array of MY_STRING and
    ...
    if (vector_struct != NULL) {
        ...
        for (i = 0; i < vector_struct->size; i++) {
            if (vector_struct->data[i] != NULL) {
                my_string_destroy(&vector_struct->data[i]);
            }
        }
...
Status vector_string_push_back(Vector_String_Ptr hVectorString, MY_STRING hString){
    ...
    hVectorString->data[hVectorString->size] = my_string_init_c_string(my_string_c_str(hString));
    (hVectorString->size)++;
    return SUCCESS;
}
...
Status vector_string_resize(Vector_String_Ptr hVectorString){
    ...
    MY_STRING* temp = (MY_STRING*)vector_string_make_temp(hVectorString, hVectorString->size);
    if (temp) {
...
void* vector_string_make_temp(Vector_String_Ptr hVectorString,unsigned int old_size){
    MY_STRING* temp = (MY_STRING*)malloc(sizeof(MY_STRING)*hVectorString->capacity);
    if (temp) {
        int j;
...
MY_STRING* vector_string_at(Vector_String_Ptr hVectorString, int index){
    if (hVectorString == NULL) {
        return NULL;
    }
    if ( index < 0 || index > hVectorString->size) {
        ...
        MY_STRING* pString = &(hVectorString->data[index]);
        return pString;
    }
}
...
void vector_string_load_one_word_of_size(unsigned int size, Vector_String_Ptr hVectorString){
    ...
    while (my_string_extraction( word, file)) {
        if (my_string_get_size(word) == size) {
