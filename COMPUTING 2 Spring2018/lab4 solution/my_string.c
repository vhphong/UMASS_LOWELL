...
Status my_string_concat(MY_STRING hString1, MY_STRING hString2) 
{
	struct S_MyString *pString1 = (struct S_MyString*) hString1;
	struct S_MyString *pString2 = (struct S_MyString*) hString2;
	char* c = NULL;
	int i = 0;
	if (pString2->size == 0)
	{
	...
	else
	{
		...
		{
			c = my_string_at(hString2, i);
			my_string_push_back(pString1, c[0]);
		}
		return SUCCESS;
	}
}
