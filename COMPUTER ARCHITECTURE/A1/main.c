...
int main(int argc, char * argv[])
{
    ...
    bit_string[0] = float_32.bit.b31?'1':'0'; // sign bit
    
    bit_string[2] = float_32.bit.b30?'1':'0'; // start of exponent bit
    ...
    bit_string[10] = float_32.bit.b23?'1':'0'; // end of exponent bit
    
    bit_string[12] = float_32.bit.b22?'1':'0'; // start of mantissa bit
    ...
    bit_string[39] = float_32.bit.b0?'1':'0'; // stop of mantissa bit
    
    //create an array to hold mantissa
    char mantissa[29];
    for (j=12; j<=39; j++) {
        mantissa[j-12]=bit_string[j];
    }
    mantissa[28]='\0';
    
    //create an array to hold exponent
    char exponent[10];
    for (k=2;k<=10;k++) {
        exponent[k-2]=bit_string[k];
    }
    exponent[9]='\0';
    
    //print out the results
    printf("the floating value for %f is broken out as:\n", float_32.floating_value_in_32_bits);
    printf("mantissa: \t0x%x  \t or: %s\n", float_32.f_bits.mantissa, mantissa);
    printf("exponent: \t0x%x      \t or: %s\n", float_32.f_bits.exponent, exponent);
