...
int main(int argc, char * argv[])
{
	...
	// Equalize two exponents then compute the number of bits of mantissa to be shifted
	
	shift_count = (exp_s1 >= exp_s2)? (exp_s1 - exp_s2) : (exp_s2 - exp_s1);
	if (shift_count > 24) shift_count = 24;
	
	if (exp_s1 > exp_s2) // if the exponent of floating point number 1 is greater than the floating point number 2's
	{		
		// shift the mantissa of the Smaller number to the right
		mant_s2 = mant_s2 >> shift_count;

		// sum up two mantissas
		...
		else{ // the hidden bit carried out
			float_32_rslt.f_bits.sign = 0;
	...
	else if (exp_s1 == exp_s2) // if the exponent of floating point number 1 equals to the floating point number 2's
	{
		// sum up two mantissas
		mant_res = mant_s1 + mant_s2;
	...
	else // if the exponent of floating point number 1 is smaller than the floating point number 2's
	{
		// shift the mantissa of the Smaller number to the right
	...
	// check if the exponent is infinity
	if (float_32_rslt.f_bits.exponent == 0xff) float_32_rslt.f_bits.mantissa = 0;
	
	// print the specs of two numbers
